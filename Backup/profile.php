...
<form name="login" method="get" >
 ...
 <p for="fname"><i class="fa fa-user"></i> User Name</p>
  <input type="text" value="<?php echo $row["user_name"]; ?>" id="fname" name="firstname" placeholder="John M. Doe" disabled>

<div style="margin-left:450px; margin-top:-105px;">
	<p for="email" ><i class="fa fa-envelope"></i> Email</p>
	  <input type="email" value="<?php echo $row["user_email"]; ?>" id="email" name="email" placeholder="kong@gmail.com" disabled>
</div>

<p >Mobile Number </p>
	<input type="text" value="<?php echo $row["phone_num"]; ?>" name="phone_num" placeholder="eg:01134567890" pattern="[0-9]{10,11}" title = "Minimum 10 numbers."required>
...
<h4 id="address" style="text-decoration:none;">ADDRESSS DETAILS</h4>
...
if(isset($_GET["update_btn"]))
{
	...
	$udob = $_GET['dob'];
	mysqli_query($connect,"update user set user_email='$uemail' where user_id='$user_id'");
	mysqli_query($connect,"update user set phone_num='$uphone_num' where user_id='$user_id'");	
	mysqli_query($connect,"update user set dob='$udob' where user_id='$user_id'");		
	?>
