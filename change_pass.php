...
*/
	...
	document.getElementById("alert").innerHTML = "not same";
	output = false;
	} 	
	return output;
}
...
<p class="pass">Current Password <span style="margin-left:15px;">:</span> <input type="password"  name="current_pass" id="currentPassword" pattern="^(?=.*[a-z])(?=.*[A-Z])(?=.*[0-9])(?=.*[!@#$%^&*_=+-]).{8,16}$" oninvalid="this.setCustomValidity('At least one uppercase letter, lowercase letter and numeric value with the minimum range from 8 to 16 characters')" oninput="this.setCustomValidity('')" required></p>
<input type="checkbox" style="width:15px; height:15px; margin-left:180px;" onclick="currentPass()"> <span style="font-style:italic; font-weight:bold; color:grey;">Show Password</span>

<p class="pass">New Password <span style="margin-left:30px;">:</span> <input type="password" name="new_pass" id="newPassword" pattern="^(?=.*[a-z])(?=.*[A-Z])(?=.*[0-9])(?=.*[!@#$%^&*_=+-]).{8,16}$" oninvalid="this.setCustomValidity('At least one uppercase letter, lowercase letter and numeric value with the minimum range from 8 to 16 characters')" oninput="this.setCustomValidity('')" min=8 required></p>
<input type="checkbox" style="width:15px; height:15px; margin-left:180px;" onclick="newPass()"> <span style="font-style:italic; font-weight:bold; color:grey;">Show Password</span>

<p class="pass">Confirm Password : <input type="password" name="confirm_pass" id="confirmPassword" class="confirmPassword" pattern="^(?=.*[a-z])(?=.*[A-Z])(?=.*[0-9])(?=.*[!@#$%^&*_=+-]).{8,16}$" oninvalid="this.setCustomValidity('At least one uppercase letter, lowercase letter and numeric value with the minimum range from 8 to 16 characters')" oninput="this.setCustomValidity('')" min=8 required><span id="alert"></span></p>
<input type="checkbox" style="width:15px; height:15px; margin-left:180px;" onclick="confirmPass()"> <span style="font-style:italic; font-weight:bold; color:grey;">Show Password</span>
...
<br>
<br>
<a href="forgetpass.php" name="forgot_btn"  >Forgot Password</a>
</div>             
</form>
...
if(isset($_POST["update_btn"]))
{
	...
	if($cpass == $row["user_pass"] && $cpass == $npass )
	{
		...
		else if($_POST["current_pass"] == $row["user_pass"] )
		{
			if($npass == $confirm_password ) {
				if(strlen($npass) < 8){
