...
<body class="fix-header card-no-border fix-sidebar">
            ...
            <div class="container-fluid">
                ...
                <div class="row page-titles">
                    <div class="col-md-5 align-self-center">
                        <h3 class="text-themecolor">Restore List</h3>
                        <ol class="breadcrumb">
                            <li class="breadcrumb-item"><a href="index.php">Dashboard</a></li>
                            <li class="breadcrumb-item"><a href="insert-product-type.php">INSERT PRODUCT TYPE</a></li>
	...
	<table class="pro-list">
		<thead>
			<tr>
			<?php
				$result = mysqli_query($connect, "select * from product WHERE product_isDelete=1");	
                    if(mysqli_num_rows($result) > 0)
					{
						?>
                <th style="padding-right:20px; padding-bottom:20px;">Product id</th>
                <th style="padding-bottom:20px; padding-left:120px;">Product Name</th>	
				<th style="padding-bottom:20px; padding-left:30px;">Product price</th>	
                <th style="padding-left:70px; padding-bottom:20px;">Action</th>
			</tr>
		</thead>
		<tbody>
			 <?php
                    ...
                    ?>			
						...
						{
							var option;
							option=confirm("Do you want to restore this product?");
							return option;
						}
						...
						{
							var option;
							option=confirm("Do you want to delete this product forever?The related product type will deleted too!");
							return option;
						}
						</script>
                    <tr>
                    <td style="padding-left:30px; padding-bottom:50px;"><?php echo $row["product_id"] ?></td>
                    <td style="padding-left:30px; padding-bottom:50px;"><?php echo $row["product_name"] ?></td>
                    <td style="padding-left:40px; padding-bottom:50px;"><?php echo $row["product_price"] ?></td>
                    <td style="padding-left:30px; padding-bottom:50px;"><a href="restore-list.php?delete&procode=<?php echo $p_row['product_id'];?>" onclick="return del_confirmation();">Delete</a></td>
					<td style="padding-right:20px; padding-bottom:50px;"><a href="restore-list.php?res&procode=<?php echo $row['product_id'];?>" onclick="return confirmation()">Restore</a></td>
					</tr>
                    <?php
					...
					{
						?>
						<p style="text-align:center; margin-top:200px; font-size:25px; font-weight:bold; color:#504A4B;">THERE IS NOTHING NEED TO BE RESTORE.</p>
...
{
	$pcode=$_GET['procode'];
	mysqli_query($connect,"UPDATE product SET product_isDelete=0 where product_id='$pcode'");
	
	?>
	<script>
	alert("One product has been restored!");
	window.location.assign("restore-list.php");
	</script>
	<?php
}
...
{
	$pcode=$_GET["procode"];
	mysqli_query($connect,"delete from product where product_id='$pcode'");
	mysqli_query($connect,"delete from product_type where product_id='$pcode'");
?>
	<script>
	alert("This product has been delete!");
	window.location.assign("restore-list.php");
	</script>
	<?php
}
