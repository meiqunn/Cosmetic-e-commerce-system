...
<head>
    ...
    <title>52Hz | Update Product Details</title>
    <!-- Favicon icon -->
                ...
                <div class="row page-titles">
                    <div class="col-md-5 align-self-center">
                        ...
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item"><a href="pro-list.php">Product List</a></li>
                                ...
                                <div class="table-responsive">
                                    <?php
                                        if(isset($_GET["edit"]))
                                        {
                                            ...
                                            $result = mysqli_query($connect, "select product_type.*,product.* from product_type,product where product_type.product_type_id='$pcode' and product_type.product_id=product.product_id ");
                                            $row = mysqli_fetch_assoc($result);
                                        }
                                    ?>
                                    ...
                                    <form name="updatefrm" method="post" action="">
                                        ...
                                        <p style="display: inline;">Product Id: <input type="text" name="prod_code"  value="<?php echo $row['product_id'];?>"  disabled > <div style="margin-left:100px;margin-bottom:20px;display:inline-block" > Product type id: <input type="text" name="prod_code"  value="<?php echo $row['product_type_id'];?>"  disabled ></div>
                                        <p>Name: <input type="text" name="product_name"  value="<?php echo $row['product_name'];?>" disabled size="100" >
                                        <p>Description: </p>
										<p><textarea name="product_des" rows="4" cols="100"><?php echo $row['product_description'];?> </textarea></p>
                                        <p>categoty: <input type="text" name="product_categoty" value="<?php echo $row['product_category'];?>" disabled>
                                            
										<p>Product type name: <input type="text" name="product_type_name" value="<?php echo $row['product_type_name'];?>" size="80">
                                        <p>Price(RM): <input type="text" name="product_price" value="<?php echo $row['product_type_price'];?>">
...
if (isset($_POST['savebtn'])) 	
{
	...
	$pname=$_POST['product_type_name'];
	$pprice=$_POST['product_price'];
	$pqty=$_POST['product_quantity'];
	$pdes = $_POST['product_des'];
	
	if($pname == $row['product_type_name'] && $pprice == $row['product_type_price'] && $pqty == $row['product_stock'])
	{
		?>
			<script>
			alert("No changes happen!");
			window.location.href="edit.php?edit&procode=<?php echo $pcode ?>";
			</script>
	<?php
	}
	else
	...
	$getproductid = mysqli_query($connect, "select * from product_type where product_type_id = '$pcode'");
	$resultproductid = mysqli_fetch_assoc($getproductid);
	$id = $resultproductid['product_id'];
	
	mysqli_query($connect,"UPDATE product_type SET product_stock='$pqty' , product_type_name='$pname',product_type_price='$pprice' where product_type_id='$pcode'");
	mysqli_query($connect, "UPDATE product SET product_description='$pdes' where product_id='$id'");
