...
if(isset($_POST['btn_submit']) )	
{
	...
	if($pcode == "" && $pname == "" && $pdes == "" && $pprice == "" && $_FILES['choosefile']['name']=="")
	{
		?>
			<script>
			alert("Please fill it up the respective details.");
			</script>
		<?php
	}
	else if($pcode == "")
	...
	if($repeat==0 && $coderepeat == 0)
	{
			...
			mysqli_query($connect, "INSERT INTO product(product_id, product_category, product_name, product_description, product_price, img_dir) VALUES ('$pcode', '$cat','$pname','$pdes', '$pprice', '$img')");
			?>
			<script>
